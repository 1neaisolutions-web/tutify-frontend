...
const CARDS_SLOT_MIN_HEIGHT = 400
/** Eased crossfade white UI → dark magic canvas. */
const WHITE_OUT_START = HOLD_COMPLETE_END
...
export const Scene07_Personalization: React.FC = () => {
  ...
  const whiteUiOp =
    frame < WHITE_OUT_START
      ? 1
      : interpolate(frame, [WHITE_OUT_START, WHITE_OUT_END], [1, 0], {
  ...
  const magicBgOp =
    frame < WHITE_OUT_START
      ? 0
      : interpolate(frame, [WHITE_OUT_START, WHITE_OUT_END], [0, 1], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
          easing: easeOut,
        })
  const loadingStackOp =
    frame < CARD_SWAP_START
      ? 1
      : interpolate(frame, [CARD_SWAP_START, CARD_SWAP_END], [1, 0], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
          easing: easeInOut,
        })
  const loadingStackScale = interpolate(loadingStackOp, [0, 1], [0.97, 1], { extrapolateRight: 'clamp' })

  const completeRevealOp =
    frame < CARD_SWAP_START
      ? 0
      : interpolate(frame, [CARD_SWAP_START, CARD_SWAP_END], [0, 1], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
          easing: easeInOut,
        })
  const completeHoldOp = frame >= CARD_SWAP_END && frame < WHITE_OUT_START ? 1 : 0
  const completeStackOp =
    frame < WHITE_OUT_START
      ? Math.max(completeRevealOp, completeHoldOp)
      : interpolate(frame, [WHITE_OUT_START, WHITE_OUT_END], [1, 0], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
          easing: easeOut,
        })
  const completeStackScale = interpolate(completeStackOp, [0, 1], [0.97, 1], { extrapolateRight: 'clamp' })
  ...
  /* Analysis UI — hold at 100%, then crossfade to magic typography */
  return (
    <AbsoluteFill style={{ overflow: 'hidden', opacity: master }}>
      {magicBgOp > 0 ? (
        <AbsoluteFill style={{ background: DARK_GRADIENT, opacity: magicBgOp }} />
      ) : null}
      <AbsoluteFill
        style={{
          background: '#F8FAFC',
      ...
      >
        ...
        <div
          style={{
            ...
            maxWidth: CONTENT_MAX_WIDTH,
            position: 'relative',
            minHeight: CARDS_SLOT_MIN_HEIGHT,
          }}
        >
        {frame >= ANALYSIS_START && loadingStackOp > 0.01 ? (
          <div
            style={{
              position: 'absolute',
              left: 0,
              ...
              flexDirection: 'column',
              gap: 16,
              opacity: loadingStackOp,
              transform: `scale(${loadingStackScale})`,
          ...
          >
            ...
            >
              ...
              <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 14, lineHeight: 1.7, color: '#334155' }}>
                ...
                {frame >= ANALYSIS_END - 30 && frame < CARD_SWAP_START ? (
        ...
        {completeStackOp > 0.01 ? (
          <div
            style={{
              ...
              flexDirection: 'column',
              gap: 12,
              opacity: completeStackOp,
              transform: `scale(${completeStackScale})`,
              transformOrigin: 'center top',
            }}
          >
            <div
