...
const LINE1_START = 8
const LINE1_ROW_GAP = 6
const LINE1_WORD_STAGGER = 6
const LINE1_SETTLE = 12
const LINE2_WORD_STAGGER = 6
const LINE2_SETTLE = 14
...
const LINE2_START = LINE1_FADE_OUT - 2
...
const RevealWord: React.FC<{
  ...
  const y = interpolate(raw, [0, 1], [22, 0], { extrapolateRight: 'clamp' })
...
export const VisionCopy: React.FC<VisionCopyProps> = ({ fontFamily, fadeOut }) => {
  ...
  const line1Op = interpolate(
    frame,
    [LINE1_FADE_OUT, LINE1_FADE_OUT + 6],
    [1, 0],
    { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' },
  )
  const showLine1 = frame < LINE1_FADE_OUT + 4

  const line2Starts = LINE2_WORDS.map((_, i) => LINE2_START + i * LINE2_WORD_STAGGER)
  const line2Op = interpolate(frame, [LINE2_START, LINE2_START + 6], [0, 1], {
...
/** Pills + scene exit after line 2 */
export const VISION_SCENE_DURATION = VISION_LINE2_DONE + 42
