...
export const SCENE04_DURATION = 360
...
const GENERATE_CLICK = 126
const SLIDE_START = 136
const GENERATING_END = 252
...
const PROC_STEPS = [
  ...
  { text: 'Building quiz & worksheet items …', delay: 158 },
  { text: 'Formatting lesson plan template …', delay: 174 },
]
...
export const Scene04_AIAssistant: React.FC = () => {
  ...
  const fadeOut = interpolate(frame, [338, 360], [1, 0], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' })
  ...
  const charCount = Math.floor(
    interpolate(frame, [32, 98], [0, PROMPT_TEXT.length], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }),
  )
  ...
  const badgeP = spring({ frame: Math.max(0, frame - 262), fps, config: theme.spring.snappy })
  const badgeOp = interpolate(badgeP, [0, 1], [0, 1]) * fadeOut

  const captionOp = interpolate(frame, [278, 300], [0, 1], { extrapolateLeft: 'clamp', extrapolateRight: 'clamp' }) * fadeOut
  ...
  return (
    <AbsoluteFill style={{ overflow: 'hidden' }}>
      <AbsoluteFill style={{ opacity: fgAlpha }}>
        <svg style={{ position: 'absolute', inset: 0, opacity: 0.04, pointerEvents: 'none' }} width={1920} height={1080}>
          <defs>
        ...
        >
          ...
          >
            ...
            />
            <span style={{ ...labelStyle, color: theme.colors.primary, fontSize: 11 }}>AI Content Generator</span>
          </div>
        </div>
        ...
        >
          <UICard
            width={INPUT_W}
            height={INPUT_H}
            glowColor={`rgba(91,79,207,${0.09 + cardClickGlow * 0.14})`}
            borderRadius={24}
            style={{
              boxShadow:
                cardClickGlow > 0.05
                  ? `0 0 0 3px rgba(91,79,207,${cardClickGlow * 0.25}), 0 12px 40px rgba(91,79,207,${0.12 + cardClickGlow * 0.15})`
                  : undefined,
            }}
          >
        ...
        {frame >= GENERATE_CLICK + 6 && slideToLeft > 0.2 && (
          <div style={{ position: 'absolute', left: inputRightX + 36, top: inputTop + 40, width: 300, display: 'flex', flexDirection: 'column', gap: 14, zIndex: 3 }}>
            {PROC_STEPS.map((step, i) => {
              const stepOp = interpolate(frame, [step.delay, step.delay + 12, 228, 242], [0, 1, 1, 0], {
                extrapolateLeft: 'clamp',
                extrapolateRight: 'clamp',
              })
              ...
              return (
                <div key={i} style={{ opacity: stepOp, transform: `translateY(${stepY}px)`, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        >
          <KineticText
            text="Powered by AI. Guided by educators."
            mode="spring-word"
            startFrame={278}
            fontSize={30}
            fontWeight={500}
