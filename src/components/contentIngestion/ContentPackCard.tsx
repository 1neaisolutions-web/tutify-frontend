    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <h3 className="text-lg font-semibold text-gray-900">{pack.name || t('content.pack.untitled')}</h3>
          </div>
        ...
        <div className="flex items-center space-x-1">
          {onEdit && (
            <button
              ...
              title={t('content.pack.editTooltip')}
            >
          ...
          {onDelete && (
            <button
              ...
              title={t('content.pack.deleteTooltip')}
            >
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-gray-500">
        <div className="flex items-center space-x-1">
          ...
          <span>{t('content.pack.documentCount', { count: pack.document_count || 0 })}</span>
        </div>
        <div className="flex items-center space-x-1">
          <Calendar className="w-4 h-4" />
          <span>{pack.created_at ? new Date(pack.created_at).toLocaleDateString() : t('shared.notAvailable')}</span>
        </div>
      </div>
    </div>
  )
