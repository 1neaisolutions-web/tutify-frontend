  ...
  const lessons: LessonContent[] = [
    {
      ...
      completed: false,
      content: {
        description: 'Transform assessments into engaging game experiences.',
        keyPoints: [
          'Reduce test anxiety through game elements',
          'Provide immediate feedback',
          'Make assessment feel like play',
    ...
    },
    {
      ...
      completed: false,
      content: {
        article: `# Validating Game-Based Assessment

## Ensuring Assessment Quality

While gamification makes assessments more engaging, we must ensure they still accurately measure learning.

### Key Principles

**Validity**: Does the assessment measure what it claims to measure?
**Reliability**: Are results consistent and dependable?
**Fairness**: Does it provide equal opportunity for all students?
**Authenticity**: Does it reflect real-world application?
...
- Balance fun with rigor`,
    ...
    },
    {
      ...
      completed: false,
      content: {
        description: 'Design gamified assessments that are both engaging and valid.',
        steps: [
    ...
    },
    {
      ...
      completed: false,
      content: {
        description: 'Access templates for various gamified assessment types.',
        sections: [
          'Quiz Game Templates',
          'Escape Room Assessments',
          'Simulation-Based Assessments',
          'Portfolio Game Templates',
          'Peer Review Games',
        ],
      },
    },
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-rose-600 via-pink-600 to-red-600 rounded-3xl p-8 text-white shadow-xl">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg transition ${
                      isActive ? 'bg-rose-50 border-2 border-rose-300' : 'border-2 border-transparent hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center ${
                        isCompleted ? 'bg-green-100 text-green-600' : isActive ? 'bg-rose-100 text-rose-600' : 'bg-gray-100 text-gray-400'
                      }`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <p className={`text-sm font-semibold truncate ${isActive ? 'text-rose-900' : 'text-gray-900'}`}>
                            {lesson.title}
                          </p>
                        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
                ...
                <ol className="space-y-3">
                  {currentLessonData.content.steps.map((step: string, idx: number) => (
                    <li key={idx} className="flex items-start gap-3 text-sm text-gray-700">
                      <span className="flex-shrink-0 w-6 h-6 rounded-full bg-rose-600 text-white flex items-center justify-center text-xs font-bold">
                        {idx + 1}
                      </span>
                      <span>{step}</span>
                    </li>
            ...
            <div className="mt-8 flex items-center justify-between pt-6 border-t border-gray-200">
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-3 bg-rose-600 text-white text-sm font-semibold rounded-full hover:bg-rose-700 transition"
              >
...
export default GamifiedAssessment
