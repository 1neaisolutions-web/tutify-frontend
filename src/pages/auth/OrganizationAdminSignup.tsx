...
const OrganizationAdminSignup: React.FC = () => {
  ...
  const validateStep2 = () => {
    const newErrors: Record<string, string> = {};
    if (!formData.organization_name) newErrors.organization_name = t('signup.validation.organizationNameRequired');
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
  ...
  const validateStep3 = () => {
    ...
    if (!formData.is_institution_admin && !formData.institution_admin_email) {
      newErrors.institution_admin_email = t('signup.validation.institutionAdminEmailRequired');
    }
    setErrors(newErrors);
    return Object.keys(newErrors).length === 0;
  };
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-bold text-green-600 mb-4">{t('signup.organization.successTitle')}</h2>
          <p className="text-gray-600 mb-4">{t('signup.organization.successBody')}</p>
          <button
            onClick={() => {
              navigate('/login');
            }}
          ...
          >
            {t('signup.organization.goToLogin')}
          </button>
        </div>
      </div>
    );
      ...
      <div className="max-w-2xl w-full bg-white rounded-lg shadow-md p-8 relative">
        ...
        <div className="bg-gradient-to-r from-amber-400 to-orange-500 text-white px-6 py-4 rounded-t-lg -mt-8 -mx-8 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-lg font-bold">🚀 {t('signup.organization.comingSoonTitle')}</h3>
              <p className="text-sm opacity-90">{t('signup.organization.comingSoonBody')}</p>
            </div>
          </div>
        </div>
        ...
        <h2 className="text-2xl font-bold text-gray-900 mb-6">{t('signup.organization.title')}</h2>
        ...
        {step === 1 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold mb-4">{t('signup.organization.stepAccount')}</h3>
            <div>
        ...
        {step === 2 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold mb-4">{t('signup.organization.stepOrganization')}</h3>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('signup.organization.organizationName')} *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('signup.organization.organizationSlug')}</label>
              <input
                type="text"
                name="organization_slug"
                value={formData.organization_slug}
                onChange={handleChange}
                placeholder={t('signup.organization.slugPlaceholder')}
                className="w-full px-3 py-2 border border-gray-300 rounded-md"
              />
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <h3 className="text-lg font-semibold mb-4">{t('signup.organization.stepInstitution')}</h3>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('signup.organization.institutionName')} *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('signup.organization.institutionType')} *</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('signup.organization.institutionSlug')}</label>
              <input
                type="text"
            ...
            <div className="flex items-center">
              ...
              <label htmlFor="is-institution-admin" className="ml-2 block text-sm text-gray-700">
                {t('signup.organization.isInstitutionAdmin')}
              </label>
            </div>
            ...
            {!formData.is_institution_admin && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  {t('signup.organization.institutionAdminEmail')} *
                </label>
                <input
                  type="email"
                  ...
                  placeholder={t('signup.organization.institutionAdminEmailPlaceholder')}
                />
        ...
        <div className="mt-6 flex gap-4">
          ...
          >
            {step === 3 ? t('signup.organization.comingSoon') : t('signup.organization.next')}
          </button>
        </div>
        ...
        >
          {t('signup.organization.backToSignup')}
        </button>
      </div>
    </div>
  );
